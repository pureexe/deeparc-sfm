...
std::vector<Point3d> DeepArcManager::readPoint3d(std::ifstream &file, int size){
    ...
    for(i = 0; i < size; i++){
        file >> x >> y >> z >> r >> g >> b;
        points.push_back(Point3d(x,y,z,r,g,b));
...
void DeepArcManager::buildParameterBlock(
    ...
    for(i = 0; i < block_size; i++){
        params[i].intrinsic(intrinsics[params[i].intrinsic_id()]);
        params[i].point3d(point3ds[params[i].point3d_id()]);
        if(arc_size != 0){
            params[i].arc(
                extrinsics[
                    params[i].pos_arc()
            ...
            params[i].ring(extrinsics[
                this->extrinsicRingIdOnHemisphere(params[i].pos_ring(),arc_size)
            ]);
            params[i].share_extrinsic(true);
        }else{
            params[i].extrinsic(
                extrinsics[
                    params[i].extrinsic_id()
                ]
            );
            params[i].share_extrinsic(false);
        }
    }
}

std::vector< std::vector<Camera> > DeepArcManager::buildCameraShare(
        int arc_size,
        int ring_size,
        std::vector<Intrinsic> &intrinsics,
        std::vector<Extrinsic> &extrinsics){
    std::vector< std::vector<Camera> > hemisphere;
    int arc_position, ring_position;
    for (arc_position = 0; arc_position < arc_size; arc_position++){
        hemisphere.push_back(std::vector<Camera>());
        for(ring_position = 0; ring_position < ring_size; ring_position++){
            hemisphere[arc_position].push_back(Camera(
...
std::vector<Camera> DeepArcManager::buildCamera(
    ...
    block_size = params.size();
    for(i = 0; i < block_size; i++){
        extrinsic_intrinsic[params[i].extrinsic_id()] = params[i].intrinsic_id();
    }
